...
def build_hdf5(
        save_file, video_file, nerual_file, label_file=None, pose_algo=None, batch_size=128, xpix=None,
    ...
    neural_data = h5py.File(nerual_file,'r')
    neural_data = np.asarray(neural_data['C_pick']).astype(np.float32) #valid_C_200
    ...
    with h5py.File(save_file, 'w', libver='latest', swmr=True) as f:
        ...
        for tr_idx, trial in enumerate(trials):
            ...
            group_n.create_dataset(
                'trial_%04i' % tr_idx, data=neural_data[ts_idxs], dtype='float32')
...
if __name__ == '__main__':
    # dst_file ='G:\\BBNC\\xiaoguihua\\data\\xiaoguihua\\gonogo\\mice376\\dust\\data.hdf5'
    avi_file ='G:\\BBNC\\xiaoguihua\\data\\20210910xiaoguihuaai148dNo go-no-go mice 376_1_d4\\Reaglined MRI video-cut.avi'
    # neural_file = "G:\\BBNC\\xiaoguihua\\data\\20210910xiaoguihuaai148dNo go-no-go mice 376_1_d4\\valid_C_pick_r0.01.mat"
    # build_hdf5(dst_file,avi_file)
    idxs = [1,2,3,4,5]
    video_cap = cv2.VideoCapture(avi_file)
    frames_tmp = get_frames_from_idxs(video_cap,idxs)
    frames_tmp = [cv2.resize(f[0], (210, 128))[None, None, ...] for f in frames_tmp]
    data_temp=np.vstack(frames_tmp).astype(np.float)
    tiff_file = 'G:\\BBNC\\xiaoguihua\\data\\20210910xiaoguihuaai148dNo go-no-go mice 376_1_d4\\Reaglined MRI video-cut.tiff'
    data_new = tifffile.imread(tiff_file)[:,np.newaxis,:,:]
    data_temp_new  = data_new[idxs].astype(np.float)
    print(np.array_equal(data_temp,data_temp_new))
    print(np.average(data_temp-data_temp_new))
    print(data_temp_new.shape)
