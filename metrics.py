...
def multi_roi_loss(output,label,rois,criterions):
    loss_dict={}
    shape = output.shape
    assert(label.shape == shape)
    assert(len(shape)==4)
    overall_loss =0
    for (cri_w,cri_name,criterion) in criterions:
        temp_loss = 0
        loss_dict[cri_name]={}
        for (roi_w,roi_name,roi) in rois:
            instant_loss =  roi_w * (criterion(output[:,:,roi[1,0]:roi[1,1],roi[0,0]:roi[0,1]],
                            label[:,:,roi[1,0]:roi[1,1],roi[0,0]:roi[0,1]]))
            loss_dict[cri_name][roi_name] = instant_loss.item()
            temp_loss += instant_loss
        overall_loss += cri_w * temp_loss
    return overall_loss,loss_dict


class PSNR(torch.jit.ScriptModule):
    __constants__ = ['data_range']
    ...
    def forward(self, img1, img2):
        ...
        return 20 * torch.log10(self.data_range / torch.sqrt(mse)).mean()
